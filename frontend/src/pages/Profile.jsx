...
import { User, Key, MapPin, Plus, Trash2, Edit } from 'lucide-react';
...
const Profile = () => {
  ...
  const handleAddressSubmit = async (e) => {
    ...
    try {
      let res;
      if (editingAddressId) {
        res = await updateAddress(editingAddressId, addrData);
      } else {
    ...
    <div className="bg-brand-cream min-h-screen py-12 font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white rounded-xl border border-brand-creamText/15 p-6 shadow-luxury h-fit">
            <span className="font-serif font-bold text-base text-brand-blue-deep block border-b border-brand-cream-dark pb-3 mb-4 flex items-center gap-1.5">
              <User size={18} className="text-brand-creamText" />
              Account Settings
            </span>
            ...
            <form onSubmit={handleUpdateProfile} className="space-y-4">
              ...
              <div className="pt-2 border-t border-brand-cream-dark">
                <span className="text-xs text-gray-400 font-bold flex items-center gap-1 mb-3">
                  <Key size={14} className="text-brand-creamText" />
              ...
              <button
                ...
                className="w-full bg-brand-blue hover:bg-brand-blue-deep text-white font-sans text-xs font-bold py-2.5 rounded-lg border border-brand-creamText/25 transition-colors disabled:opacity-50"
              >
          ...
          <div className="lg:col-span-2 bg-white rounded-xl border border-brand-creamText/15 p-6 shadow-luxury">
            ...
            {addingAddress ? (
              ...
              <form onSubmit={handleAddressSubmit} className="space-y-4 bg-brand-cream/20 p-4 border border-brand-creamText/10 rounded-lg">
              ...
              user.addresses && user.addresses.length > 0 ? (
                    ...
                    >
                      ...
                      <div className="flex justify-end gap-2 border-t border-brand-cream-dark mt-4 pt-3">
                        <button
                          ...
                          className="text-gray-500 hover:text-brand-creamText p-1"
                          title="Edit Address"
                        >
                          <Edit size={14} />
                        </button>
                ...
                <div className="text-center py-10 border border-dashed border-brand-creamText/20 rounded-lg">
                  <p className="text-xs text-gray-500 font-sans mb-3">No addresses found. Add an address to facilitate shopping checkouts.</p>
                  <button
                    onClick={() => setAddingAddress(true)}
                    className="bg-brand-blue text-white text-xs font-bold px-5 py-2 rounded-full border border-brand-creamText/25"
                  >
