...
import { ClipboardList, Calendar, DollarSign, Activity, Eye, X } from 'lucide-react';
...
const OrderHistory = () => {
  ...
  const getTrackingPercent = (status) => {
    switch (status) {
  ...
  return (
    <div className="bg-brand-cream min-h-screen py-12 font-sans relative">
          ...
          <div className="bg-white w-full max-w-2xl rounded-xl overflow-hidden shadow-2xl border border-brand-creamText/20 animate-scale-up">
            ...
            <div className="bg-brand-blue px-6 py-4 flex items-center justify-between border-b border-brand-creamText/25 text-white">
              <div className="flex flex-col">
                <span className="font-serif font-bold text-sm text-brand-creamText">Order Details</span>
                <span className="text-[10px] text-brand-cream/80 font-mono">ID: {selectedOrder._id}</span>
              </div>
              <button 
                onClick={() => setSelectedOrder(null)} 
                className="text-brand-creamText hover:text-white p-1"
              >
                <X size={20} />
              </button>
            </div>
            ...
            {/* Modal Body */}
            <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto pr-2 custom-scrollbar">
              ...
              {selectedOrder.orderStatus !== 'Cancelled' ? (
                <div>
                  ...
                  <div className="w-full bg-gray-150 h-2 rounded-full overflow-hidden">
                    <div className={`bg-gradient-to-r from-brand-blue to-brand-creamText h-full ${getTrackingPercent(selectedOrder.orderStatus)}`} />
                  </div>
              ...
              <div className="border border-brand-creamText/15 p-4 rounded-lg bg-brand-cream/20 text-xs text-gray-600">
                    ...
                    <div key={item._id} className="py-3 flex items-center justify-between text-xs">
                      <div className="flex items-center space-x-3">
                        <img 
                          ...
                          alt="" 
                          className="h-12 w-10 object-cover rounded border border-brand-creamText/10" 
                        />
                        <div>
            ...
            <div className="bg-brand-cream/30 px-6 py-3 text-right border-t border-brand-cream-dark">
              <button 
                onClick={() => setSelectedOrder(null)} 
                className="bg-brand-blue hover:bg-brand-blue-deep text-white text-xs font-semibold px-5 py-2 rounded-lg border border-brand-creamText/20"
              >
                Close details
              </button>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {orders.length === 0 ? (
          <div className="bg-white border border-brand-creamText/15 rounded-xl p-16 text-center shadow-luxury max-w-md mx-auto">
            ...
            <Link
              to="/shop"
              className="inline-block bg-brand-blue hover:bg-brand-blue-deep text-white text-xs font-semibold px-6 py-2.5 rounded-full border border-brand-creamText/20"
            >
              Shop Collection
            </Link>
          </div>
          ...
          <div className="space-y-4">
            {orders.map((order) => (
              <div 
                key={order._id} 
                className="bg-white rounded-xl border border-brand-creamText/15 p-5 shadow-luxury hover:border-brand-creamText/30 transition-all duration-200 grid grid-cols-1 md:grid-cols-5 gap-4 items-center"
              >
                ...
                <div className="md:col-span-2 space-y-1">
                  ...
                  <div className="flex items-center space-x-1.5 text-xs text-gray-600">
                    <Calendar size={13} className="text-brand-creamText" />
                    <span>Placed on {new Date(order.createdAt).toLocaleDateString()}</span>
                  </div>
                  <div className="flex items-center space-x-1.5 text-xs text-gray-600">
                    <DollarSign size={13} className="text-brand-creamText" />
                ...
                <div className="text-right">
                  <button
                    ...
                    className="w-full md:w-auto bg-brand-cream hover:bg-brand-creamText/15 text-brand-blue-deep text-xs font-bold py-2 px-4 rounded-lg border border-brand-creamText/20 flex items-center justify-center gap-1.5 transition-colors"
                  >
                    <Eye size={13} />
