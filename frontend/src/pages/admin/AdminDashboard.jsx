  ...
  return (
    <div className="space-y-8 font-sans">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        {/* Revenue */}
        <div className="bg-white rounded-xl border border-brand-creamText/15 p-5 shadow-luxury flex items-center justify-between">
          <div>
        ...
        <div className="bg-white rounded-xl border border-brand-creamText/15 p-5 shadow-luxury flex items-center justify-between">
          ...
          <div className="h-12 w-12 bg-amber-50 border border-amber-200 text-brand-creamText rounded-lg flex items-center justify-center">
            <Package size={22} />
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-brand-creamText/15 p-6 shadow-luxury">
              ...
              {monthlySales.map((item, idx) => {
                ...
                return (
                  <div key={idx} className="flex flex-col items-center group w-12">
                    {/* Tooltip */}
                    <div className="opacity-0 group-hover:opacity-100 absolute bottom-full mb-1 bg-brand-blue text-brand-creamText text-[10px] py-1 px-2 rounded shadow-md border border-brand-creamText/20 transition-opacity whitespace-nowrap z-25">
                      ₹{item.revenue.toLocaleString('en-IN')}
                    </div>
                    {/* Bar */}
                    <div 
                      style={{ height: `${Math.max(5, heightPercent)}%` }}
                      className="w-8 bg-gradient-to-t from-brand-blue-deep to-brand-creamText rounded-t hover:brightness-110 shadow transition-all duration-300 cursor-pointer"
                    />
                    {/* Label */}
        ...
        <div className="bg-white rounded-xl border border-brand-creamText/15 p-6 shadow-luxury flex flex-col justify-between">
          <div>
          ...
          <div className="border-t border-brand-cream-dark pt-4 mt-4 space-y-3">
            <Link
              to="/admin/products"
              className="flex items-center justify-between text-xs text-brand-blue hover:text-brand-creamText font-semibold transition-colors"
            >
      ...
      <div className="bg-white rounded-xl border border-brand-creamText/15 p-6 shadow-luxury">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link
            to="/admin/orders"
            className="flex items-center space-x-1 text-xs text-brand-blue hover:text-brand-creamText font-semibold transition-colors"
          >
