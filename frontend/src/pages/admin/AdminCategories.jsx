    ...
    <div className="space-y-6 font-sans">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 px-4">
          <div className="bg-white w-full max-w-md rounded-xl overflow-hidden shadow-2xl border border-brand-creamText/20 animate-scale-up">
            
            {/* Modal Header */}
            <div className="bg-brand-blue px-6 py-4 flex items-center justify-between border-b border-brand-creamText/25 text-white">
              <span className="font-serif font-bold text-sm text-brand-creamText">{editingId ? 'Edit Category' : 'Create Category'}</span>
              ...
              <div className="pt-2">
                ...
                {imagePreview && (
                  <div className="mt-3">
                    ...
                    <img src={imagePreview} alt="" className="h-20 w-36 object-cover rounded border border-brand-creamText/10" />
                  </div>
                )}
                ...
                {!imagePreview && existingImage && (
                  <div className="mt-3">
                    ...
                    <img src={existingImage} alt="" className="h-20 w-36 object-cover rounded border border-brand-creamText/10" />
                  </div>
                )}
              </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {categories.map((cat) => (
            <div 
              key={cat._id}
              className="bg-white rounded-xl border border-brand-creamText/15 p-4 shadow-luxury flex flex-col justify-between"
            >
              <div className="space-y-3">
                <div className="aspect-[16/9] w-full bg-brand-cream rounded overflow-hidden border border-brand-creamText/10">
