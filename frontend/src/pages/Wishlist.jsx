        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {wishlist.map((product) => {
            ...
            return (
              <div 
                key={product._id} 
                className="bg-white rounded-lg border border-brand-creamText/15 overflow-hidden shadow-luxury hover:border-brand-creamText/30 transition-all duration-350 flex flex-col justify-between"
              >
                {/* Product Image */}
                <div className="relative aspect-[4/5] bg-brand-cream overflow-hidden">
                  <button
                    onClick={() => removeFromWishlist(product._id)}
                    className="absolute top-3 right-3 z-10 bg-white/90 text-red-700 p-2 rounded-full shadow-md border border-brand-creamText/15"
                ...
                <div className="p-4 flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <Link to={`/product/${product._id}`}>
                      <h3 className="font-serif font-bold text-brand-blue-deep text-xs sm:text-sm line-clamp-2 leading-snug hover:text-brand-creamText">
                        {product.name}
                      </h3>
                    </Link>
                  </div>
                  ...
                  <div className="mt-4 pt-3 border-t border-brand-cream-dark flex flex-col gap-3">
                    ...
                    {product.stock > 0 ? (
                      <button
                        ...
                        className="w-full bg-brand-blue hover:bg-brand-blue-deep text-white font-sans text-xs font-semibold py-2 rounded-full border border-brand-creamText/20 flex items-center justify-center space-x-1.5 transition-colors"
                      >
