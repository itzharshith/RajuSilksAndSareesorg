...
export default function AdminOrdersPage() {
  ...
  const openDetailsModal = (ord: Order) => {
    ...
    <div className="space-y-6 font-sans">
      ...
      {selectedOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 px-4">
          <div className="bg-white w-full max-w-2xl rounded-xl overflow-hidden shadow-2xl border border-brand-cream-text/20 animate-scale-up text-gray-700">
            ...
            <div className="bg-brand-blue px-6 py-4 flex items-center justify-between border-b border-brand-cream-text/25 text-white">
              <div className="flex flex-col">
                <span className="font-serif font-bold text-sm text-brand-cream-text">Order Administration</span>
                <span className="text-[10px] text-brand-cream/80 font-mono">ID: {selectedOrder._id}</span>
              </div>
              <button onClick={() => setSelectedOrder(null)} className="text-brand-cream-text hover:text-white p-1">
                <X size={20} />
              </button>
            </div>

            {/* Form */}
            <form onSubmit={handleUpdateStatusSubmit} className="p-6 space-y-5 max-h-[75vh] overflow-y-auto pr-2">
              
              {/* Status Update Fields */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-brand-cream/20 p-4 border border-brand-cream-text/15 rounded-lg">
                <div>
                  ...
                  <select
                    ...
                    className="w-full text-xs bg-white border border-brand-cream-text/15 rounded-lg p-2 focus:outline-none focus:border-brand-cream-text"
                  >
                ...
                <div className="space-y-2">
                  {selectedOrder.products.map((item) => (
                    <div key={item._id} className="flex justify-between items-center text-xs">
                      <span>{item.product?.name || 'Deleted Product'} <span className="text-gray-400 font-sans">x{item.quantity}</span></span>
              ...
              {/* Total Summary */}
              <div className="border-t border-brand-cream-dark pt-3 flex justify-between items-center text-xs text-gray-600">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-brand-cream-dark">
                ...
                <button
                  ...
                  className="bg-brand-blue text-white text-xs font-bold px-6 py-2 rounded-lg border border-brand-cream-text/25 flex items-center gap-1.5"
                >
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-brand-cream-text/15 shadow-luxury overflow-hidden">
                  ...
                  <tr key={ord._id} className="hover:bg-brand-cream/15">
                    ...
                    <td className="px-6 py-4 text-right">
                      <button
                        ...
                        className="bg-brand-cream hover:bg-brand-cream-text/15 text-brand-blue-deep text-xs font-bold py-1.5 px-3 rounded-lg border border-brand-cream-text/20 flex items-center gap-1 inline-flex"
                      >
                        <Edit size={12} />
                        <span>Manage</span>
                      </button>
          ...
          <div className="md:hidden divide-y divide-gray-100">
            {orders.map((ord) => (
              <div key={ord._id} className="p-4 space-y-3 hover:bg-brand-cream/5 text-xs text-gray-600">
                ...
                <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                  ...
                  <div>
                    <button
                      onClick={() => openDetailsModal(ord)}
                      className="bg-brand-cream hover:bg-brand-cream-text/15 text-brand-blue-deep text-[11px] font-bold py-1.5 px-3 rounded-lg border border-brand-cream-text/20 flex items-center gap-1 inline-flex transition-colors"
                    >
                      <Edit size={12} />
                      <span>Manage</span>
                    </button>
